...
struct sync_connection_base_impl {
 protected:
  ...
  void read_body_normal(Socket& socket_,
    ...
    while (asio::read(socket_, response_buffer, asio::transfer_at_least(
                                                    1), error)) {
      body_stream << &response_buffer;
    }
  }
  ...
  void read_body_transfer_chunk_encoding(
    ...
    if (empty(content_length_range)) {
      typename headers_range<
          basic_response<Tag>>::type transfer_encoding_range =
          ...
          if (chunk_size == 0) {
            ...
            do {
              if (chunk_size != 0) {
                std::size_t chunk_bytes_read =
                    read(socket_,
                         response_buffer,
                         asio::transfer_at_least(chunk_size),
                         error);
            ...
            if (chunk_size != 0)
              throw std::runtime_error(
                  "Size mismatch between tranfer encoding chunk data size and declared chunk size.");
          }
        } while (!stopping);
      } else
      ...
      if (already_read)
        body_stream << &response_buffer;
      size_t length = lexical_cast<
                          size_t>(boost::begin(content_length_range)->second) -
                      already_read;
      if (length == 0)
        return;
      size_t bytes_read = 0;
      while ((bytes_read = asio::read(socket_,
                                      response_buffer,
  ...
  void read_body(Socket& socket_,
    ...
    if (version_major == 1 && version_minor == 0) {
      ...
      if (response_.version() == "HTTP/1.0")
        ...
        read_body_transfer_chunk_encoding(socket_,
                                          response_,
                                          response_buffer,
                                          body_stream);
    } else {
...
struct sync_connection_base {
  ...
  static sync_connection_base<Tag,
                              version_major,
                              version_minor>* new_connection(
      resolver_type& resolver,
      resolver_function_type resolve,
      bool https,
      optional<string_type> const& cert_filename = optional<string_type>(),
  ...
  virtual void send_request_impl(string_type const& method,
                                 basic_request<Tag> const& request_) = 0;
