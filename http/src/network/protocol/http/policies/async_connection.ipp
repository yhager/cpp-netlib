...
shared_ptr<response_base> simple_async_client_connection::send_request(
  ...
  connection_.reset(new (std::nothrow)
                    impl::async_connection(resolver_delegate_,
                                           follow_redirects_,
                                           connection_delegate_)) if (
      !connection_.get())
...
network::http::simple_async_connection_manager(
    bool cache_resolved,
    bool follow_redirects,
...
boost::shared_ptr<
    network::http::client_connection> network::http::simple_async_connection_manager::get_connection(
  ...
  if (cache_resolved_) {
    ...
    if (!this->shared_resolver_delegate.get())
      this->shared_resolver_delegate.reset(new (std::nothrow)
                                           async_resolver_delegate(service));
  ...
  connection_delegate = connection_delegate_factory::new_connection_delegate(
                            service,
                            openssl_certificate_,
                            openssl_verify_path_);
  connection.reset(new (std::nothrow)
                   simple_async_client_connection(follow_redirects_,
                                                  resolver_delegate,
                                                  connection_delegate));
  if (!connection.get())
    BOOST_THROW_EXCEPTION(std::runtime_error("Insufficient memory."));
...
void http_1_1_async_connection::reset() { connection_.reset(); }
