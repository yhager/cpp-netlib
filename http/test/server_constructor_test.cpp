...
BOOST_AUTO_TEST_CASE(minimal_constructor) {
  ...
  options.address("127.0.0.1").port("80");

  BOOST_CHECK_NO_THROW(sync_server sync_instance(options, sync_handler));
  BOOST_CHECK_NO_THROW(
      async_server async_instance(options, async_handler, pool));
}
...
BOOST_AUTO_TEST_CASE(with_io_service_parameter) {
  ...
  http::server_options options;
  options.address("127.0.0.1").port("80").io_service(&io_service);
...
BOOST_AUTO_TEST_CASE(with_socket_options_parameter) {
  ...
  options.address("127.0.0.1").port("80").reuse_address(true)
      .report_aborted(true).receive_buffer_size(4096).send_buffer_size(4096)
      .receive_low_watermark(1024).send_low_watermark(1024)
      .non_blocking_io(true).linger(true).linger_timeout(0);
