...
TEST(client_test, http_get_test) {
    ...
    ASSERT_TRUE ( status_message_ == std::string("Found") || status_message_ == std::string("OK") );
}
...
TEST(client_test, http_get_test_timeout_1_0) {
    ...
    ASSERT_THROW ( response_ = client_.get(request); temp = body(response_); , std::exception );
}
