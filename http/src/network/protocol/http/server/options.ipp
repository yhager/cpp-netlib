...
class server_options_pimpl {
 public:
  ...
  void address(std::string const& address) { address_ = address; }
  ...
  void io_service(boost::asio::io_service* service) { io_service_ = service; }

  boost::asio::io_service* io_service() const { return io_service_; }

  void reuse_address(bool setting) { reuse_address_ = setting; }

  bool reuse_address() const { return reuse_address_; }

  void report_aborted(bool setting) { report_aborted_ = setting; }
  ...
  void send_buffer_size(int buffer_size) { send_buffer_size_ = buffer_size; }
  ...
  int receive_low_watermark() const { return receive_low_watermark_; }

  void send_low_watermark(int buffer_size) {
    send_low_watermark_ = buffer_size;
  }

  int send_low_watermark() const { return send_low_watermark_; }

  void non_blocking_io(bool setting) { non_blocking_io_ = setting; }

  bool non_blocking_io() const { return non_blocking_io_; }

  void linger(bool setting) { linger_ = setting; }

  bool linger() const { return linger_; }

  void linger_timeout(int setting) { linger_timeout_ = setting; }

  int linger_timeout() const { return linger_timeout_; }
 ...
 private:
  ...
  int receive_buffer_size_,
      ...
      send_low_watermark_,
      linger_timeout_;
  bool reuse_address_, report_aborted_, non_blocking_io_, linger_;
...
server_options::~server_options() { delete pimpl_; }
...
std::string const server_options::address() const { return pimpl_->address(); }
...
std::string const server_options::port() const { return pimpl_->port(); }

server_options& server_options::io_service(
...
bool server_options::reuse_address() const { return pimpl_->reuse_address(); }
...
bool server_options::report_aborted() const { return pimpl_->report_aborted(); }
...
bool server_options::linger() const { return pimpl_->linger(); }
...
int server_options::linger_timeout() const { return pimpl_->linger_timeout(); }
