...
struct file_cache {
  ...
  typedef std::map<std::string,
                   std::vector<server::response_header>> meta_map;
  ...
  bool add(std::string const& path) {
    ...
    if (regions.find(real_filename) != regions.end())
      return true;
#ifdef O_NOATIME
  ...
  boost::iterator_range<std::vector<server::response_header>::iterator> meta(
    ...
    if (headers != file_headers.end()) {
      std::vector<server::response_header>::iterator begin =
          headers->second.begin(), end = headers->second.end();
...
struct file_server {
  ...
  void operator()(server::request const& request,
    ...
    if (request.method == "HEAD") {
      ...
      static server::response_header error_headers[] = { { "Connection",
                                                           "close" } };
      connection->set_status(server::connection::not_supported);
      connection->set_headers(boost::make_iterator_range(error_headers,
                                                         error_headers + 1));
