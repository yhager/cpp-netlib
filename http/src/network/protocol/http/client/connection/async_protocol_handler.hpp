...
struct http_async_protocol_handler {
 protected:
  ...
  void init_response(response& response_) {
    ...
    boost::shared_future<
        std::multimap<std::string, std::string>> headers_future(
        headers_promise.get_future());
    headers(response_, headers_future);
    boost::shared_future<std::string> body_future(body_promise.get_future());
  ...
  logic::tribool parse_version(Delegate& delegate_,
    ...
    if (parsed_ok == true) {
      ...
      part_begin = part.begin();
      delegate_->read_some(asio::mutable_buffers_1(part.c_array(), part.size()),
                           callback);
    }
    return parsed_ok;
  }
