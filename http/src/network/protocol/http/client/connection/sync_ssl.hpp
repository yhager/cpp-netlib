...
struct https_sync_connection
  ...
  typedef sync_connection_base_impl<Tag,
                                    version_major,
                                    version_minor> connection_base;
  ...
  https_sync_connection(
      ...
      optional<string_type> const& certificate_filename = optional<
                                                              string_type>(),
  ...
  void init_socket(string_type const& hostname, string_type const& port) {
    connection_base::init_socket(socket_.lowest_layer(),
                                 resolver_,
                                 hostname,
                                 port,
                                 resolve_);
  ...
  void close_socket() {
    ...
    socket_.lowest_layer()
        .shutdown(asio::ip::tcp::socket::shutdown_both, ignored);
    if (ignored)
      return;
