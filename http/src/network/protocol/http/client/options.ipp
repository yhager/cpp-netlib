...
class client_options_pimpl {
 public:
  ...
  void follow_redirects(bool setting) { follow_redirects_ = setting; }

  bool follow_redirects() const { return follow_redirects_; }

  void cache_resolved(bool setting) { cache_resolved_ = setting; }
...
client_options& client_options::io_service(
...
bool client_options::cache_resolved() const { return pimpl->cache_resolved(); }

client_options& client_options::add_openssl_certificate_path(
...
std::list<
    std::string> const& client_options::openssl_certificate_paths() const {
  return pimpl->openssl_certificate_paths();
}

client_options& client_options::add_openssl_verify_path(
    std::string const& path) {
...
client_options& client_options::connection_manager(
    std::shared_ptr<http::connection_manager> manager) {
  pimpl->connection_manager(manager);
  return *this;
}

std::shared_ptr<
    http::connection_manager> client_options::connection_manager() const {
  return pimpl->connection_manager();
}

client_options& client_options::connection_factory(
    std::shared_ptr<http::connection_factory> factory) {
  pimpl->connection_factory(factory);
  return *this;
}

std::shared_ptr<
    http::connection_factory> client_options::connection_factory() const {
...
class request_options_pimpl {
 public:
  request_options_pimpl() : timeout_ms_(30 * 1000), max_redirects_(10) {}

  request_options_pimpl* clone() const {
    return new (std::nothrow) request_options_pimpl(*this);
  }

  void timeout(uint64_t milliseconds) { timeout_ms_ = milliseconds; }

  uint64_t timeout() const { return timeout_ms_; }

  void max_redirects(int redirects) { max_redirects_ = redirects; }
 ...
 private:
  ...
  request_options_pimpl& operator=(
      request_options_pimpl);  // cannot be assigned.
};
...
request_options::~request_options() { delete pimpl; }
...
uint64_t request_options::timeout() const { return pimpl->timeout(); }
...
int request_options::max_redirects() const { return pimpl->max_redirects(); }
