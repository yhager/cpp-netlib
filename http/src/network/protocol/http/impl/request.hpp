...
template <>
struct headers_container<http::tags::http_server> : vector<
    http::tags::http_server>::apply<
    http::request_header<http::tags::http_server>::type> {}
;

template <>
struct headers_container<http::tags::http_async_server> : vector<
    http::tags::http_async_server>::apply<
    http::request_header<http::tags::http_async_server>::type> {}
;
...
template <class Tag> struct basic_request : public basic_message<Tag> {
 ...
 public:
  ...
  explicit basic_request(network::uri const& uri_) : uri_(uri_) {}

  void uri(string_type const& new_uri) { uri_ = new_uri; }

  void uri(network::uri const& new_uri) { uri_ = new_uri; }
...
template <> struct request_headers_wrapper<tags::http_async_server> {
  ...
  request_headers_wrapper(
      basic_request<tags::http_async_server> const& request_)
      : request_(request_) {}
  typedef headers_container<
      tags::http_async_server>::type headers_container_type;
  operator headers_container_type() { return request_.headers; }
}
;
