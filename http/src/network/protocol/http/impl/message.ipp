...
typename message_impl<Tag>::string_type const message_impl<
  ...
  if (has_max_age) {
    ...
    set_cookie_header += boost::lexical_cast<
                             message_impl<Tag>::string_type>(max_age);
    set_cookie_header += '\"';
  }
...
bool message_impl<Tag>::base64_decode(
  ...
  static const char decoding_data[] = { nop,
                                        nop,
                                        nop,
                                        nop,
                                        nop,
  ...
  for (unsigned int i = 0; i < input_length; i++) {
    ...
    if (!(
        ++i <
        input_length))  // we need at least two input bytes for first byte output
      return false;
