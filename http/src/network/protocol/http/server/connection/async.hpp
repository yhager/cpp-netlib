...
class async_server_connection
    : public std::enable_shared_from_this<async_server_connection> {
 public:
  enum status_t {
    ok = 200,
    created = 201,
 ...
 private:
  static char const* status_message(status_t status) {
    static char const ok_[] = "OK", created_[] =
                              "Created", accepted_[] =
                              "Accepted", no_content_[] =
                              "No Content", multiple_choices_[] =
                              "Multiple Choices", moved_permanently_[] =
                              "Moved Permanently", moved_temporarily_[] =
                              "Moved Temporarily", not_modified_[] =
                              "Not Modified", bad_request_[] =
                              "Bad Request", unauthorized_[] =
                              "Unauthorized", forbidden_[] =
                              "Fobidden", not_found_[] =
                              "Not Found", not_supported_[] =
                              "Not Supported", not_acceptable_[] =
                              "Not Acceptable", internal_server_error_[] =
                              "Internal Server Error", not_implemented_[] =
                              "Not Implemented", bad_gateway_[] =
                              "Bad Gateway", service_unavailable_[] =
                              "Service Unavailable", unknown_[] = "Unknown";
    switch (status) {
      case ok:
        return ok_;
      case created:
 ...
 public:
  ...
  template <class Range> void write(Range const& range) {
    ...
    std::function<void(boost::system::error_code)> f =
        std::bind(&async_server_connection::default_error,
                  async_server_connection::shared_from_this(),
                  std::placeholders::_1);
  ...
  typename std::enable_if<
      !std::is_base_of<boost::asio::const_buffer,
                       typename Range::value_type>::value>::type write(
      Range const& range,
  ...
  typename std::enable_if<
      std::is_base_of<boost::asio::const_buffer,
                      typename ConstBufferSeq::value_type>::value>::type write(
      ConstBufferSeq const& seq,
      Callback const& callback) {
 ...
 public:
  ...
  typedef std::function<void(input_range,
 ...
 private:
  ...
  typedef std::shared_ptr<
      std::vector<boost::asio::const_buffer>> shared_buffers;
      ...
      switch (state) {
        case method:
          ...
          boost::fusion::tie(parsed_ok, result_range) =
              parser.parse_until(request_parser::method_done, input_range);
          if (!parsed_ok) {
            client_error();
            break;
        ...
        case uri:
          input_range = boost::make_iterator_range(new_start, data_end);
          boost::fusion::tie(parsed_ok, result_range) =
              parser.parse_until(request_parser::uri_done, input_range);
          if (!parsed_ok) {
            client_error();
            break;
          } else if (parsed_ok == true) {
        ...
        case version:
          input_range = boost::make_iterator_range(new_start, data_end);
          boost::fusion::tie(parsed_ok, result_range) =
              parser.parse_until(request_parser::version_done, input_range);
          if (!parsed_ok) {
            client_error();
            break;
          } else if (parsed_ok == true) {
        ...
        case headers:
          input_range = boost::make_iterator_range(new_start, data_end);
          boost::fusion::tie(parsed_ok, result_range) =
              parser.parse_until(request_parser::headers_done, input_range);
          if (!parsed_ok) {
            client_error();
            break;
          } else if (parsed_ok == true) {
  ...
  void write_impl(Range range,
    ...
    shared_buffers buffers = std::make_shared<
                                 std::vector<boost::asio::const_buffer>>(0);
